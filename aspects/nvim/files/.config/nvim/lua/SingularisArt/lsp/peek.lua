...
local function create_floating_file(location, opts)
  ...
  local contents = vim.api.nvim_buf_get_lines(
    ...
    range.start.line,
    math.min(
      range["end"].line + 1 + (opts.context or SingularisArt.lsp.config.peek.max_height),
      range.start.line + (opts.max_height or SingularisArt.lsp.config.peek.max_height)
    ),
    false
  )
  if next(contents) == nil then
    print("peek: Unable to get contents of the file!", vim.log.levels.WARN)
    return
  end
  local width, height = vim.lsp.util._make_floating_popup_size(contents, opts)
  local if_nil = vim.F.if_nil
  opts = vim.lsp.util.make_floating_popup_options(
    if_nil(width, SingularisArt.lsp.config.peek.max_width),
    if_nil(height, SingularisArt.lsp.config.peek.max_height),
    opts
  )
...
local function preview_location_callback(result)
  ...
  local opts = {
    border = "rounded",
    context = SingularisArt.lsp.peek.context,
  }
...
function M.open_file()
  ...
  if not filepath then
    print("peek: Unable to open the file!", vim.log.levels.ERROR)
    return
  end
...
function M.Peek(what)
  ...
  if vim.tbl_contains(vim.api.nvim_list_wins(), M.floating_win) then
    ...
    if not success_1 then
      print("peek: You cannot edit the current file in a preview!", vim.log.levels.ERROR)
      return
    end
    ...
    vim.api.nvim_buf_set_keymap(
      ...
      "n",
      "<CR>",
      ":lua require('SingularisArt.lsp.peek').open_file()<CR>",
