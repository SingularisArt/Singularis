...
local update_statusline = function(default, action)
  ...
  elseif filetype == "qf" then
    if action == "blur" then
      result = "%{v:lua.SingularisArt.statusline.gutterpadding()}"
        .. " "
        .. " "
        .. " "
        .. " "
      ...
      result = vim.g.SingularisArtQuickfixStatusline or ""
    end
  else
    result = 1
  end
...
--   local blurred = "%{v:lua.SingularisArt.statusline.gutterpadding()}"
...
statusline.load = function()
  ...
  vim.opt.statusline = ""
    ...
    .. "%{v:lua.SingularisArt.statusline.lhs()}" -- Red/green/orange modified/activity status.
    ...
    .. "%{v:lua.SingularisArt.statusline.fileprefix()}" -- Relative path to file's directory.
    ...
    .. "%{v:lua.SingularisArt.statusline.filetype()}" -- Filetype (not using %Y because I don't want caps).
    .. "%{v:lua.SingularisArt.statusline.fileencoding()}" -- File-encoding if not UTF-8.
    ...
    .. "%{v:lua.SingularisArt.statusline.rhs()}" -- Line/column info.
    .. "%*" -- Reset highlight group.
end
