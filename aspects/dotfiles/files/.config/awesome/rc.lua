...
local hotkeys_popup = require("awful.hotkeys_popup").widget require("awful.hotkeys_popup.keys") local my_table      = awful.util.table or gears.table -- 4.{0,1} compatibility
...
local browser      = os.getenv("BROWSER") or "chromium-browser"
...
globalkeys = my_table.join(
    ...
    awful.key({ modkey, "Shift" }, ",",
        ...
        {description = "mpc seek 0%", group = "audio"}),
    awful.key({ modkey, "Shift" }, ".",
        function ()
            os.execute("mpc repeat")
            beautiful.mpd.update()
        end,
        {description = "mpc repeat", group = "audio"}),
    awful.key({ modkey, }, "[",
        function ()
            os.execute("mpc seek +10")
            beautiful.mpd.update()
        end,
        {description = "mpc seek +10", group = "audio"}),
    awful.key({ modkey, }, "]",
        function ()
            os.execute("mpc seek -10")
            beautiful.mpd.update()
    ...
    awful.key({ modkey, "Shift", }, "[",
        function ()
            os.execute("mpc seek +60")
            beautiful.mpd.update()
        end,
        {description = "mpc seek +60", group = "audio"}),
    awful.key({ modkey, "Shift", }, "]",
        function ()
            os.execute("mpc seek -60")
            beautiful.mpd.update()
